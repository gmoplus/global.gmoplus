...
<form method="post" action="{$rlBase}index.php?controller={$cInfo.Controller}" onsubmit="return submitHandler();">
    ...
    <tr>
        <td style="width: 108px; border-right: 1px #667835 solid;" align="right" valign="top">
            {foreach from=$configGroups item='group' name='fGroups'}
                {if empty($group.Plugin_status) || $group.Plugin_status == 'active'}
                    <div id="ltab_{$group.ID}" title="{$group.name}" class="ltab {if $smarty.foreach.fGroups.first}tabs_active{else}tabs_inactive{/if}" {if $smarty.foreach.fGroups.first}style="margin: 0;"{/if} onclick="location.href='{$rlBase}index.php?controller={$cInfo.Controller}&group={$group.ID}'"><div>{$group.name}</div></div>
            ...
            if (typeof sKey === 'undefined') {
                var firstTab = document.querySelector('.ltab');
                if (firstTab && firstTab.id.indexOf('ltab_') === 0) {
                    var parts = firstTab.id.split('_');
                    if (parts.length > 1) {
                        window.sKey = parts[1];
                    }
                }
        ...
        <td valign="top">
            {foreach from=$configGroups item='group' name='fGroups'}
                <div id="larea_{$group.ID}" class="larea{if $smarty.get.group == $group.ID || $smarty.foreach.fGroups.first} {else} hide{/if}">
