...
if ($page_info['Tpl']) {
    ...
    if ($page_info['Login'] && !defined('IS_LOGIN')) {
        ...
        // Debug log for access denied
        $debug_info = "Access denied for page: " . $page_info['Key'] . "\n";
        $debug_info .= "User Type ID: " . ($account_info['Type_ID'] ?? 'null') . "\n";
        $debug_info .= "Page Deny: " . $page_info['Deny'] . "\n";
        $debug_info .= "User Abilities for this page: " . ($account_info['Abilities'][$page_info['Key']] ?? 'not set') . "\n";
        file_put_contents('debug_access_denied.log', date('Y-m-d H:i:s') . " - " . $debug_info . "\n", FILE_APPEND);
