...
if (defined('IS_LOGIN')) {
    ...
    if ($_GET['nvar_1'] == $search_results_url ||
        ...
        // Her listing için teklif sayısını ekle (arama modunda)
        if ($listings) {
            $listing_ids = array();
            ...
            if ($listing_ids) {
                ...
                if ($check_table) {
                    $quote_counts_sql = "SELECT Listing_ID, COUNT(*) as quote_count 
                                       FROM `{db_prefix}quote_requests` 
                                       WHERE Listing_ID IN (" . implode(',', $listing_ids) . ")
                                       GROUP BY Listing_ID";
                    $quote_counts_result = $rlDb->getAll($quote_counts_sql);
                    
                    $quote_counts = array();
                    foreach ($quote_counts_result as $row) {
                        $quote_counts[$row['Listing_ID']] = $row['quote_count'];
                    }
                    
                    // Her listing'e teklif sayısını ekle
                    foreach ($listings as $key => $listing) {
                        $listings[$key]['quote_count'] = isset($quote_counts[$listing['ID']]) ? $quote_counts[$listing['ID']] : 0;
                    }
                }
            }
        }
    ...
    /* browse mode */
        ...
        // Her listing için teklif sayısını ekle
        if ($listings) {
            $listing_ids = array();
            foreach ($listings as $key => $listing) {
                $listing_ids[] = $listing['ID'];
